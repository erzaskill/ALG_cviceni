...
void PosliData(string T, unsigned int AdrId, string fce, Polozka hod){
// T: URL (blockState, lokState); fce: název položky; hod: požadovaná hodnota
 int kod; string Param;
 GenData("odata.jsn", "lokStav", fce, hod);
 Param="/usr/bin/curl -T odata.jsn -H \"Authorization: Basic bHJrdjp5MlF0Qms5cmp1UWhmN2VG\" -H \"Content-type: application/json\" "+Server+"/"+T+"/"+IntToStr(AdrId);
 kod=system(Param.c_str());
